...
type JSONSchema2TS<T> = T extends {
  enum: infer TEnum extends readonly (string | number)[]
}
  ? TEnum[number]
  : {
      string: string
      number: number
      boolean: boolean
      array: T extends { items: infer TItems }
        ? JSONSchema2TS<TItems>[]
        : unknown[]
      object: T extends { properties: infer TProps }
        ? T extends { required: infer TRequired extends readonly string[] }
          ? Symplify<
              {
                [K in Exclude<keyof TProps, TRequired[number]>]?: JSONSchema2TS<
                  TProps[K]
                >
              } & {
                [K in Extract<
                  keyof TProps,
                  TRequired[number]
                >]-?: JSONSchema2TS<TProps[K]>
              }
            >
          : { [K in keyof TProps]?: JSONSchema2TS<TProps[K]> }
        : Record<string, unknown>
    }[(T & { type: JsonSchemaType })['type']]

type Symplify<T> = { [K in keyof T]: T[K] } & {}
